...
class StationOidcUserService(
    ...
    override fun loadUser(request: OidcUserRequest?): OidcUser {
        ...
        if (request == null) {
            throw OAuth2AuthenticationException("Login request is null!")
        }
        ...
        println("reg.method: " + request.clientRegistration.registrationId) // "google"

        println("email: $email")
        println("local user: $localUser")
